 ...
 */
public class ForwardMovingPathTest {
    ...
    @Test
    public void testMovesRideForward() {

        final ForwardMovingPath path = new ForwardMovingPath();
    ...
    @Test
    public void testMovesWalkForward() {
        final ForwardMovingPath path = new ForwardMovingPath();
        final MovementPath newPath = path.appendWalk(
                ORIGIN_STOP, TIME_AT_ORIGIN, WAYPOINT_STOP,TIME_AT_WAYPOINT, 
    ...
    @Test
    public void testAppendsToBack() {
        final MovementPath path = new ForwardMovingPath().appendWalk(
                ORIGIN_STOP, TIME_AT_ORIGIN, WAYPOINT_STOP, TIME_AT_WAYPOINT, 
                new WalkingCosts(Duration.ofSeconds(10), 10));
        ...
        Assert.assertFalse(newPath.getMovements().get(1).equals(movement));
    }

    @Test
    public void testMakesNewPath() {
        final ForwardMovingPath path = new ForwardMovingPath();
        final MovementPath newPath = path.appendWalk(
                ORIGIN_STOP, TIME_AT_ORIGIN, WAYPOINT_STOP, TIME_AT_WAYPOINT, 
                new WalkingCosts(Duration.ofSeconds(10), 10));
